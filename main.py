    ...
    def create_model(self):
        ...
        if st.session_state.create1 is None:
            ...
            if st.checkbox("Proceed to create"):
                ...
                saved_model,file_name=self.save_model(st.session_state.create1)
        else:
            ...
            self.saved_model(st.session_state.save1,st.session_state.user_input)

    def save_model(self, created_model):
        """Save the created model and provide a download link."""
        if created_model is None:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pkl") as tmp_file:
                if created_model==st.session_state.create1:
                    st.session_state.save1=reg_save(created_model, tmp_file.name)
                    tmp_file_path = tmp_file.name
                elif created_model==st.session_state.ensemble1:
                    st.session_state.save2=reg_save(created_model, tmp_file.name)
                    tmp_file_path = tmp_file.name
                elif created_model==st.session_state.tune1:
                    st.session_state.save3=reg_save(created_model, tmp_file.name)
                    tmp_file_path = tmp_file.name
                else:
                    st.session_state.save4=reg_save(created_model, tmp_file.name)
                    tmp_file_path = tmp_file.name
                    

            # Provide a download button for the model file
            file_name=st.text_input("Enter Desired Model Name")
            if st.button(f"Conform to download model with name - {file_name}.pkl {created_model}"):
            ...
            st.success("Model saved successfully. You can download it using the button above.")
            if created_model==st.session_state.create1:
                st.session_state.save1=reg_save(created_model, tmp_file.name)
                return st.session_state.save1,file_name
            elif created_model==st.session_state.ensemble1:
                st.session_state.save2=reg_save(created_model, tmp_file.name)
                return st.session_state.save2,file_name
            elif created_model==st.session_state.tune1:
                st.session_state.save3=reg_save(created_model, tmp_file.name)
                return st.session_state.save3,file_name
            else:
                st.session_state.save4=reg_save(created_model, tmp_file.name)
                return st.session_state.save4,file_name
    def saved_model(self, saved_model, file_name):
        if saved_model:
            # Display download button if the user confirms the file name
            if st.button(f"Confirm download of model as '{file_name}.pkl' {saved_model}"):
                with open(saved_model[1], "rb") as file:
                    st.download_button(
                        label="Download Model",
                        data=file,
                        file_name=f"{file_name}.pkl",
    ...
    def ensemble_model(self):
        if st.session_state.create1 is None:
            ...
            if st.session_state.ensemble1 is None:
                ...
                st.markdown("#### Saving the model")
                saved_model,file_name=self.save_model(st.session_state.ensemble1)
            else:
                ...
                st.dataframe(st.session_state.predict2_ensemble)
                st.markdown("#### Model is already saved")
                self.saved_model(st.session_state.save2,st.session_state.user_input)
    ...
    def tune_model(self):
        if st.session_state.create1 is None:
            ...
            if st.session_state.tune1 == None:
                ...
                st.markdown("#### Saving the model")
                saved_model,file_name=self.save_model(st.session_state.tune1)
            else:
                ...
                st.dataframe(st.session_state.tune2_predict)
                st.markdown("#### Model is already saved")
                self.saved_model(st.session_state.save3,st.session_state.user_input)
    ...
    def tune_ensemble(self):
        if st.session_state.ensemble1==None:
            ...
            if st.session_state.tune1_ensemble== None:
                ...
                st.markdown("#### Saving the model")
                saved_model,file_name=self.save_model(st.session_state.tune1_ensemble)
            else:
                ...
                st.dataframe(st.session_state.tune2_ensemble_prredict)
                st.markdown("#### Model is already saved")
                self.saved_model(st.session_state.save4,st.session_state.user_input)
